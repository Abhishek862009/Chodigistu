...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    console.error(err);
    note.textContent = 'Email ya password galat hai.';
...
async function loadCategories() {
  ...
  try {
    ...
    renderCategoryList();
    fillCategorySelects();
    if (categories.length === 0) empty.style.display = 'block';
    else empty.style.display = 'none';
  } catch (e) {
    console.error(e);
    showToast('Categories load nahi ho payi', true);
  }
}
...
function fillCategorySelects() {
  ...
  [uploadSel, showcaseSel].forEach(sel => {
    ...
    sel.innerHTML = categories.length
      ...
      : '<option value="">Pehle category banayein</option>';
...
document.getElementById('addCatBtn').addEventListener('click', async () => {
  ...
  if (!name) { showToast('Category ka naam likhein', true); return; }
  try {
    ...
    input.value = '';
    showToast('Category ban gayi');
    loadCategories();
  } catch (e) {
    console.error(e);
    showToast('Category ban nahi payi', true);
  }
});

async function deleteCategory(id) {
  if (!confirm('Is category ko aur iski sabhi photos ko delete karna hai?')) return;
  try {
    ...
    console.error(e);
    showToast('Delete nahi ho paya', true);
  }
}
...
document.getElementById('fileInput').addEventListener('change', async (e) => {
  ...
  if (!catId) { showToast('Pehle category chunein', true); return; }
  const files = Array.from(e.target.files);
  const progressEl = document.getElementById('uploadProgress');

  for (let i = 0; i < files.length; i++) {
    const file = files[i];
    progressEl.textContent = `Upload ho raha hai ${i + 1} / ${files.length}...`;
    try {
      ...
      console.error(err);
      showToast(`"${file.name}" upload nahi ho payi`, true);
    }
  }
...
async function loadPhotosForUpload(catId) {
  ...
  const cat = categories.find(c => c.id === catId);
  label.textContent = (cat ? cat.name : '') + ' ki photos:';
  try {
    const snap = await db.collection('photos').where('categoryId', '==', catId).orderBy('createdAt', 'desc').get();
    if (snap.empty) {
      grid.innerHTML = '<div class="empty-state">Abhi is category mein photos nahi hain.</div>';
      return;
    }
    snap.forEach(doc => {
      const p = doc.data();
    ...
    console.error(e);
    grid.innerHTML = '<div class="empty-state">Photos load nahi ho payin.</div>';
  }
}
...
async function loadPhotosForShowcase(catId) {
  ...
  try {
    const snap = await db.collection('photos').where('categoryId', '==', catId).orderBy('createdAt', 'desc').get();
    if (snap.empty) {
      grid.innerHTML = '<div class="empty-state">Is category mein photos nahi hain.</div>';
      return;
    }
    snap.forEach(doc => {
      const p = doc.data();
      ...
      tile.querySelector('input').addEventListener('change', async (e) => {
        try {
          ...
          showToast(e.target.checked ? 'Homepage par add ho gayi' : 'Homepage se hata di gayi');
        } catch (err) {
          console.error(err);
          showToast('Update nahi ho paya', true);
    ...
    console.error(e);
    grid.innerHTML = '<div class="empty-state">Load nahi ho paya.</div>';
  }
}
...
async function loadQueries() {
  ...
  try {
    ...
    console.error(e);
    wrap.innerHTML = '<div class="empty-state">Queries load nahi ho payin.</div>';
  }
}
